...
class BFSTest(unittest.TestCase):
    ...
    def test_empty_graph(self):
        # given-when
        path = self.bfs.run(self.graph.edges, None, None).get('path')
    ...
    def test_one_node_graph(self):
        ...
        path = self.bfs.run(self.graph.edges, node_id, node_id).get('path')
    ...
    def test_two_node_connected_graph(self):
        ...
        # when
        path = self.bfs.run(self.graph.edges, node_id1, node_id3).get('path')
